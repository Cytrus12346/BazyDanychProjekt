...
INSERT INTO UserRoles (user_id, role_id) VALUES
((SELECT user_id FROM Users WHERE username = 'manager_user'), (SELECT role_id FROM Roles WHERE role_name = 'menadżer firmy'));

INSERT INTO UserRoles (user_id, role_id) VALUES
((SELECT user_id FROM Users WHERE username = 'influencer_user'), (SELECT role_id FROM Roles WHERE role_name = 'influencer'));
GRANT ALL PRIVILEGES ON *.* TO 'admin_user'@'localhost';
GRANT SELECT, INSERT, UPDATE, DELETE ON `company_db`.* TO 'manager_user'@'localhost';
...
SELECT c.campaign_name, MAX(cr.revenue) AS max_revenue 
